...
# Pseudo-3D variables. These let the enemy bounce off walls
...
# lets the be enemy launched up
# horizontal launch
var launch_force: float = 175.0  
# upward launch    
var launch_upward_force: float = 100.0  

# heavy attack launches more the longer you hold it
...
func _physics_process(delta: float):
	...
	# hitstun with wall detection for wallbounces
	if in_hitstun:
		var collision = move_and_collide(velocity * delta)
		if collision:
			var collider = collision.get_collider()
...
func choose_and_execute_attack(player: Node2D):
	...
	# more powerful attacks become more likely as the number of killed enemies increases
...
func execute_heavy_barrage_start():
	...
	# enemy flashes when its charging heavy attack
...
# enemy moves for a set amount of time in set intervals
...
func take_damage(damage_amount: int, knockback_direction: Vector2, combo_stage: int, is_comboing, custom_knockback: float = 0, hit_type: String = "normal"):
	...
	# inturrupts attacks when the enemy is hit
	...
	# how much does the enemy get knocked back
	if combo_stage == 1:
		final_knockback = 20
	else:
		final_knockback = 10
